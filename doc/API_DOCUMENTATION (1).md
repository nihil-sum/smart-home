...
**所有 8 处路径/字段名不匹配问题已修复**，前端现与后端完全对齐。

| # | 问题类型 | 状态 | 修复方式 |
|---|---------|:----:|---------|
| 1 | `GET /users` → `/admin/users` | ✅ | 前端改为 `/admin/users` |
| 2 | `PUT /users/:id/toggle` → `/admin/users/:id/status` | ✅ | 前端改为 `/admin/users/:id/status` |
| 3 | `GET/PUT /settings` → `/admin/settings` | ✅ | 前端改为 `/admin/settings` |
| 4-5 | `/appointments/my|/manage` → `/appointments` | ✅ | 前端改为 `/appointments`（后端角色自动区分） |
| 6-7 | `/contracts/my|/manage` → `/contracts` | ✅ | 前端改为 `/contracts`（后端角色自动区分） |
| 8 | `rating` → `score` | ✅ | 前端改为 `score` |
| 9 | `date/time/note` → `visitDate/visitTime/contact` | ✅ | 前端字段名已对齐后端 |
...
**状态**: ✅ 已实现（路径已对齐，前端统一使用 `/appointments`）
...
**状态**: ✅ 已实现（前端已对齐使用 `/admin/users/:id/status` + `{ status }`）
...
所有 8 处前后端不一致问题已修复，无阻塞项。

| # | 问题 | 修复状态 | 涉及文件 |
|---|------|:--------:|---------|
| 1 | 前端调用 `/users`，后端是 `/admin/users` | ✅ 已修复 | `UserManage.vue` |
| 2 | 前端调用 `/users/:id/toggle` + `{ active }`，后端是 `/admin/users/:id/status` + `{ status }` | ✅ 已修复 | `UserManage.vue` |
| 3 | 前端调用 `/settings`，后端是 `/admin/settings` | ✅ 已修复 | `SystemSettings.vue` |
| 4 | 前端调用 `/appointments/my`，后端是 `/appointments` | ✅ 已修复 | `MyAppointments.vue` |
| 5 | 前端调用 `/appointments/manage`，后端是 `/appointments` | ✅ 已修复 | `AppointmentManage.vue` |
| 6 | 前端调用 `/contracts/my`，后端是 `/contracts` | ✅ 已修复 | `MyContracts.vue` |
| 7 | 前端调用 `/contracts/manage`，后端是 `/contracts` | ✅ 已修复 | `ContractManage.vue` |
| 8 | 前端发送 `rating`，后端期望 `score` | ✅ 已修复 | `MyReviews.vue` |
